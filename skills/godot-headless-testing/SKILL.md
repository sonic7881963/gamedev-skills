...
| 测试全绿，但 @tool / `scenes/verify` 下的编辑器脚本有解析错、编辑器或运行时才爆（改了个签名/少个参数漏网） | SceneTree 测试套件通常只扫 `res://scripts`，**不扫 `scenes/`、`addons/` 下的 @tool/authoring 脚本** | 改这类脚本后跑 headless `godot --headless --path <绝对路径> --check-only --script res://...`（判 EXIT+stderr；`--path` 用绝对路径，相对 `.` 会漂）。@tool/.tres authoring 的编辑器时坑另见 `godot-editor-tool-traps` |
