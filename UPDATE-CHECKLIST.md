...
`agents/` 的 9 个设计顾问 + `skills/design-advisory/` + `templates/design/` 的 12 个模板**不是受监控上游**
（没有外部 release 要追，**不要塞进上表**）。它们是从一个使用中的源项目蒸馏的**下游发布快照**：源在那个项目里
随实际使用演进，本库只是某一时刻的脱敏快照。同步方向是**单向下游**——

1. 源项目的设计部门发生**实质变更**（顾问职责/纪律改写、维度→顾问映射变动、模板结构调整、新增或退役顾问）后，
   把变更脱敏同步到本库对应文件。
2. **脱敏**：去掉源项目专属的文档路径耦合（如把私有的设计文档目录路径泛化成「你项目的设计文档目录」）、
   项目/作品专名、本机路径，只保留可泛化的顾问理论与模板。
3. 仅文案/理论增补无需 bump 版本；若顾问 frontmatter 的 `model`/`effort` 档位语义变了，在 README 注明。
